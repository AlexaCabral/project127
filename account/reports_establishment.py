...
class EstablishmentWindow:
    def __init__(self, root):
        ...
        frame_label_right = LabelFrame(
            ...
            bd=2,
            relief=RIDGE,
            text="View Establishment Details",
    ...
    def search(self):
        try:
            ...
            if str(self.var_filter_dropdown.get()) == "High Rating":
                mycursor.execute(
                    "SELECT * FROM food_establishment where average_rating >= 4 and "
                    + str(self.var_search.get())
                    + " LIKE '%"
                    + str(self.var_search_text.get())
                    + "%'"
                )
                rows = mycursor.fetchall()
                print("Query1")
                ...
                mycursor.execute(
                    "SELECT * FROM food_establishment where "
                    + str(self.var_search.get())
                    + " LIKE '%"
                    + str(self.var_search_text.get())
                    + "%'"
