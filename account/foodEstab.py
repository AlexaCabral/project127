...
def add_item():
    ...
    if dialog.result:
        establishments_data.append(dialog.result)
...
def create_new_box(establishment):
    ...
    edit_button = ttk.Button(new_box_frame, text="Edit", command=lambda: edit_item(new_box_frame, establishment))
    edit_button.pack(side="left", padx=5)

    delete_button = ttk.Button(new_box_frame, text="Delete", command=lambda: delete_box(new_box_frame))
    delete_button.pack(side="left", padx=5)

    reviews_button = ttk.Button(new_box_frame, text="Check Food Reviews", command=check_reviews)
...
def edit_item(box_frame, establishment):
    ...
    if dialog.result:
        ...
        edit_button = ttk.Button(box_frame, text="Edit", command=lambda: edit_item(box_frame, establishment))
        edit_button.pack(side="left", padx=5)

        delete_button = ttk.Button(box_frame, text="Delete", command=lambda: delete_box(box_frame))
        delete_button.pack(side="left", padx=5)
...
label1 = tk.Label(foodestabWindow, text="Food Establishment Owner Account", font=('Arial', 20, 'bold'), bg="white", fg="#FFBA00", anchor="nw")
