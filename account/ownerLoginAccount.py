...
def new_window(parent):
    ...
    def login():
        ...
        if (email == "" or email == "Email") or (password == "" or password == "Password"):
            ...
            mycursor.execute("SELECT * FROM owner WHERE email=%s and password=%s",(email, password))
    ...
    # header
    label = tk.Label(canvas, text="LOGIN", font=('Courier', 30, 'bold'), bg="white", fg="#6D9773")
    label.place(x=490, y=40)
    labelowner = tk.Label(canvas, text="OWNER", font=('Courier', 15, 'bold'), bg="white", fg="#6D9773")
    ...
    # log in button
    loginBtn = tk.Button(canvas, text="Log In", font=('Courier', 16, 'bold'), bd=0, bg="#6D9773", activebackground="#0C3B2E", fg="#2E4D3D", activeforeground="white", cursor="hand2", width=20, command=login)
    loginBtn.place(x=425, y=330)
    
    #log in as customer text
    customerLabel = tk.Label(canvas, text="Or Log in as", font=('Arial', 10), bg="white", fg="#0C3B2E")
    customerLabel.place(x=492, y=372)

    #log in as customer button
    customerBtn = tk.Button(canvas, text="Customer.", font=('Arial', 10, 'bold'), bd=0, bg="#FFFFFF", activebackground="#FFFFFF", fg="#6D9773", activeforeground="#0C3B2E", cursor="hand2", width=8, command=lambda: gotoLogIn(newWindow, parent))
    customerBtn.place(x=566, y=371)
    
    # create account
    signupLabel = tk.Label(canvas, text="Don't have an account?", font=('Arial', 10), bg="white", fg="#0C3B2E")
