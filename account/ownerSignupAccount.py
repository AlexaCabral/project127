...
def signup(parent):
                ...
                if pvalid == 0:
                    ...
                    if not is_emailValid(email):
                        ...
                        if myresult == 0:
                            ...
                            mycursor.execute("INSERT INTO owner(name, password, email) VALUES (%s, %s, %s)", (name, password, email))
    ...
    # header
    label = tk.Label(canvas, text="SIGN UP", font=('Courier', 30, 'bold'), bg="white", fg="#6D9773")
    label.place(x=470, y=40)
    labelcustomer = tk.Label(canvas, text="OWNER", font=('Courier', 15, 'bold'), bg="white", fg="#6D9773")
    ...
    pwBtn = tk.Button(canvas, text="Show", font=('Courier', 12, 'bold'), bd=0, bg="#FFFFFF", activebackground="#FFFFFF", fg="#FFBA00", activeforeground="#FF5050", cursor="hand2", command=show)
    pwBtn.place(x=680, y=270)
    
    # show/hide button for password
    cpwBtn = tk.Button(canvas, text="Show", font=('Courier', 12, 'bold'), bd=0, bg="#FFFFFF", activebackground="#FFFFFF", fg="#FFBA00", activeforeground="#FF5050", cursor="hand2", command=cshow)
    cpwBtn.place(x=680, y=310)
    
    # sign up button
    signupBtn = tk.Button(canvas, text="Sign Up", font=('Courier', 16, 'bold'), bd=0, bg="#6D9773", activebackground="#0C3B2E", fg="#2E4D3D", activeforeground="white", cursor="hand2", width=20, command=signupacc)
    signupBtn.place(x=420, y=430)
    
    # go back to log in account
    loginLabel = tk.Label(canvas, text="Already have an account?", font=('Arial', 12), bg="white", fg="#0C3B2E")
    loginLabel.place(x=420, y=590)

    createBtn = tk.Button(canvas, text="Log in.", font=('Arial', 11, 'bold'), bd=0, bg="#FFFFFF", activebackground="#FFFFFF", fg="#6D9773", activeforeground="#0C3B2E", cursor="hand2", width=6, command=lambda: gotoLogIn(signupWindow, parent))
