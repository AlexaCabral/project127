...
class ReviewWindow:
    def __init__(self, root):
        ...
        # ========== left-side ==========
        frame_label_left = LabelFrame(
            self.root,
            bd=2,
            relief=RIDGE,
            text="Establishments and Food Items",
        ...
        search_entry = ttk.Entry(
            ...
            font=("Courier", 12, "bold"),
            width=14,
            textvariable=self.var_search_establishment,
        )
        ...
        self.Establishment_Table = ttk.Treeview(
            left_frame,
            column=(
                "establishment_id",
                "location",
        ...
        search_entry = ttk.Entry(
            ...
            font=("Courier", 12, "bold"),
            width=3,
            textvariable=self.var_estab_id,
            state="readonly",
        )
        ...
        search_entry = ttk.Entry(
            ...
            font=("Courier", 12, "bold"),
            width=10,
            textvariable=self.var_search_food_item,
        )
        ...
        self.Fooditem_Table = ttk.Treeview(
            left_frame2,
            column=("item_id", "price", "description", "name", "establishment_id"),
            xscrollcommand=scroll_x1.set,
        ...
        search_entry = ttk.Entry(
            ...
            font=("Courier", 12, "bold"),
            width=4,
            textvariable=self.var_estab_id_right,
            state="readonly",
        )
        search_entry.grid(row=0, column=1, padx=2)

        # label food item ID
        food_item_id_label_right = Label(
            frame_label_right,
            text="Food Item ID:",
            font=("Courier", 12, "bold"),
            bg="#FFBA00",
        ...
        filter_search = ttk.Combobox(
            frame_label_right,
            font=("Courier", 12, "bold"),
            width=10,
            state="readonly",
            textvariable=self.var_filter_dropdown,
        )
        filter_search["value"] = (
            "",
            "January",
    ...
    def fetch_data(self):
        try:
            mydb = mysql.connector.connect(
                host="localhost", user="root", password="chancekababy2021", database="project"
            )
            ...
            if len(rows) != 0:
                self.Establishment_Table.delete(
                    *self.Establishment_Table.get_children()
                )
                for i in rows:
    ...
    def search(self):
        try:
            ...
            mycursor.execute(
                ...
                + str(self.var_search_establishment.get())
                + "%'"
            )
            rows = mycursor.fetchall()
            print("Query")
    ...
    def search_food_item(self):
        try:
            ...
            if self.var_estab_id.get() == "":
                ...
                mycursor.execute(
                    "SELECT * FROM food_item WHERE name LIKE '%"
                    + str(self.var_search_food_item.get())
                    + "%'"
                )
                rows = mycursor.fetchall()
                print("Query")
        ...
        try:
            ...
            if self.var_estab_id_right.get() == "" and self.var_food_id.get() == "":
                if self.var_filter_dropdown.get() == "":
                    ...
                    mycursor.execute(
                        query1
                        + " WHERE MONTHNAME(datetime) LIKE '"
                        + str(self.var_filter_dropdown.get())
                        + "'"
                    )
                    rows = mycursor.fetchall()
                ...
                if self.var_filter_dropdown.get() == "":
                    ...
                    mycursor.execute(
                        ...
                        + str(self.var_estab_id_right.get())
                    )
                    rows = mycursor.fetchall()
                else:
                ...
                if self.var_filter_dropdown.get() == "":
                    ...
                    mycursor.execute(
                        query1 + " WHERE item_id =" + str(self.var_food_id.get())
                    )
                    rows = mycursor.fetchall()
                else:
